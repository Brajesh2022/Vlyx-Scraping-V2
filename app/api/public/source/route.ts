...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`Public API: Starting scrape for: ${url}`);
    ...
    while (attempts < maxAttempts) {
      attempts++;
      console.log(`Public API: Attempt ${attempts}: Checking for Cloudflare challenge...`);
      ...
      if (!cloudflareDetected && stableCount >= 2) {
        console.log(`Public API: No Cloudflare challenge detected and page is stable on attempt ${attempts}`);
        break;
      }

      if (cloudflareDetected) {
        console.log(`Public API: Cloudflare challenge detected on attempt ${attempts}, waiting...`);
        ...
        try {
          ...
          console.log('Public API: Human simulation failed:', error instanceof Error ? error.message : 'Unknown error');
        }
        ...
        try {
          await Promise.race([
            page.waitForNavigation({ 
              waitUntil: 'domcontentloaded', 
              timeout: 15000 
            }).catch(() => console.log('Public API: Navigation timeout')),
    ...
    if (stillOnCloudflare) {
      console.log('Public API: Cloudflare challenge could not be bypassed after all attempts');
      return NextResponse.json({
        success: false,
        error: 'Cloudflare protection bypass failed',
        message: 'Unable to bypass the advanced Cloudflare protection on this website. The site requires human verification.',
    ...
    console.log(`Public API: Successfully scraped: ${finalUrl} (Title: ${pageTitle})`);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        source_length: pageSource.length,
      },
    ...
    return NextResponse.json({
      ...
      message: error instanceof Error ? error.message : 'An unknown error occurred while scraping the website',
  ...
  if (!url) {
    return NextResponse.json(
      { 
        ...
        message: 'Please provide a URL parameter. Example: /api/public/source?url=https://example.com',
        usage: {
          method: 'GET',
          example: '/api/public/source?url=https://example.com',
